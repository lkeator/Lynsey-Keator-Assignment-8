...
Write a function that will compute the total score for respondents on a Likert scale. Your function should accept three required arguments: a data frame with the individual item responses (as usual, individuals in rows and items in columns), a numeric with the scale used for the items (i.e. you would send "6" of if the items were on a six-point scale), and a numeric vector indicating which item numbers should be reverse scored. Your function should return a data frame that is the original data frame with an additional column added to the end that contains the score for each respondent.
...
ggplot(data = hsb2, mapping = aes(x = math, y = science, color = ses)) +
  facet_grid( female ~ ses) +
  geom_hex(bins = 50)


```
This creates multiple plots to show the relationship of math and science scores as a function of gender and SES.
